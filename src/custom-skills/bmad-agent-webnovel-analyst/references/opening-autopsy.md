...
Write the analysis to `{agent.report_output_path}/<genre>-openings-<YYYY-MM-DD>.md` with the same methodology line the playbook requires: titles, ranking source, access date, substitutions. State the exact path written on completion. If the openings can't be sampled at all, say so rather than emitting a checklist that reads as observed when it isn't.
